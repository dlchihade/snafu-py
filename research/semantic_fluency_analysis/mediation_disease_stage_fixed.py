#!/usr/bin/env python3
"""Create properly formatted disease stage-adjusted mediation figures"""
...
def create_mediation_figure_fixed(result: dict, outcome_type: str, out_path: Path):
    """Create properly formatted mediation figure"""
    colors_fig1 = setup_nature_style()
    
    # Set outcome-specific colors and labels
    if outcome_type == 'svf':
    ...
    # Perpendicular offset label at arrow midpoint
    ...
    # Value labels above/below bars - FIXED to ensure all bars have labels
    for rect, val in zip(bars, totals):
        height = rect.get_height()
        if val >= 0:
            ax_bars.text(rect.get_x() + rect.get_width()/2.0, height + 0.02,
    ...
    # Better y-limits calculation
    y_max = max(abs(min(totals)), abs(max(totals)))
    y_abs = max(0.2, y_max * 1.3)  # Increased margin for better spacing
...
def main():
    """Generate properly formatted disease stage-adjusted mediation figures"""
    print("Creating properly formatted disease stage-adjusted mediation figures...")
    ...
    if result_svf:
        create_mediation_figure_fixed(result_svf, 'svf', 
                                    output_dir / 'mediation_svf_disease_stage_fixed.png')
    ...
    if result_ee:
        create_mediation_figure_fixed(result_ee, 'ee', 
                                    output_dir / 'mediation_ee_disease_stage_fixed.png')
        print(f"EE metric mediation (disease stage adjusted): N = {result_ee['N']}")
    
    print("\nCreated properly formatted disease stage-adjusted mediation figures:")
    print(" - output/mediation_svf_disease_stage_fixed.(png|pdf)")
    print(" - output/mediation_ee_disease_stage_fixed.(png|pdf)")
