...
def main():
    ...
    out_dir = base / 'output'
    top_png = out_dir / 'mediation_svf_disease_stage_working.png'
    bottom_png = out_dir / 'mediation_ee_disease_stage_working.png'
    if not top_png.exists() or not bottom_png.exists():
        print('Missing input PNGs. Expected:', top_png, bottom_png)
        sys.exit(1)
    out_png = out_dir / 'mediation_disease_stage_working_combined.png'
    out_pdf = out_dir / 'mediation_disease_stage_working_combined.pdf'
    stack_images_vertically(top_png, bottom_png, out_png, out_pdf)
    print('Saved combined disease stage-adjusted mediation figure with exact working formatting:')
    print(' -', out_png)
    print(' -', out_pdf)
